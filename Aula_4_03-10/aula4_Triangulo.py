...
### SOLUÇÃO EXERCICIOS
# 1. glClearColor(255, 255, 255, 1)
# 2. glRotatef(r, 1, 0, 0) 
# 3. glRotatef(r, 1, 1, 0) 
# 4. glColor3f(0, 0, 0)
# 5. obtive diferentes triangulos (retos, isoceles, escaleno, acutangulo, obtusangulo)
# 6. glRotatef(r, 0, 0, -1)
# > para fazer ele rotacionar mais rápido, basta apenas aumentar o grau que ele rotaciona a cada 'frame'
# > (final do while)r += r
# 7. Ele fica centralizado no meio da tela
# 8. A alteração diretamente influencia o tamanho ocupado pelo triangulo
# 9. inverti o resultado obtido ao triggar o event.key k_a e k_d 
# >if event.type == KEYDOWN:
# >	if event.key == k_a:
# >		x+=0.2
# >	if event.key == k_d:
# >		x-=0.2
# 10. criei uma variavel Z com valor -6
# > z = -6 # Posição Z
# após alterei a função glTranslatef para conter Z
# > glTranslatef(x, y, z)  
# e depois criei os event.key para afastar e aproximar o eixo Z
# >	if event.key == K_z:  
# >		z += 0.2 
# >	if event.key == K_x:
# >		z -= 0.2 
...
z = -6 # Posição inicial no eixo Z
r = 1  # Ângulo de rotação do triângulo
...
def init():
    glClearColor(255, 255, 255, 1)  # Define a cor de fundo (R, G, B, Alpha)
...
def draw():
    ...
    # z → deslocamento no eixo Z (afasta ou aproxima da tela)
    ...
    glColor3f(0, 0, 0)  # Define a cor do triângulo (R, G, B)
...
def main():
    ...
    global x, y, z, r, ex, ey, ez  # Declara as variáveis globais para uso dentro da função
        ...
        for event in pygame.event.get():  # Captura eventos do sistema (teclado, mouse, etc.)
            ...
            if event.type == KEYDOWN:  # Se uma tecla for pressionada
                ...
                if event.key == K_z: # Se a tecla Z for pressionada
                    z += 0.2 # Move o triângulo para frente
                if event.key == K_x: # Se a tecla X for pressionada
                    z -= 0.2 # Move o triângulo para trás
